...
def get_ld_variants(species, variant_id, population, d_prime, r2, window):
    url = f"{ENSEMBL_API}/ld/{species}/{variant_id}/{population}?d_prime={d_prime};r2={r2};window_size={window}"
...
if phenotype_input and population_input:
    ...
    if st.session_state.variant_list is None or st.session_state.ld_storage is None or st.session_state.linked_pheno_map is None:
        ...
        d_prime = st.slider("d_prime", 0.00, 1.00, 1.00)
        r2 = st.slider("r2", 0.00, 1.00, 0.85)
        window_size = st.slider("window size", 0, 500, 500)
        ...
        for i, var in enumerate(variant_list):
            ...
            ld_variants = get_ld_variants("homo_sapiens", var, population_input, d_prime, r2, window_size)
            if ld_variants:
